import { MODULE_GROUPS, MODULE_LABELS } from '../../constants/adminModules';

export function AdminModulePermissionsFields({ selectedModules = [], onChange, error }) {
  const selected = new Set(selectedModules);

  const toggleModule = (key) => {
    const next = new Set(selected);
    if (next.has(key)) {
      if (key === 'ADMIN_DASHBOARD' && next.size === 1) return;
      next.delete(key);
    } else {
      next.add(key);
    }
  ...
  const toggleGroup = (keys, selectAll) => {
    ...
    keys.forEach((key) => {
      ...
      else if (key !== 'ADMIN_DASHBOARD' || next.size > 1) next.delete(key);
    });
    if (next.size === 0) next.add('ADMIN_DASHBOARD');
    onChange([...next]);
  };
    ...
    <section className="space-y-4 border-b border-gray-100 pb-5">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Choose which super admin modules this team member can view and access.
        </p>
      </div>
...
export function AdminModuleAccessList({ moduleAccess = [] }) {
  if (!moduleAccess.length) {
    return <p className="text-sm text-gray-500">Full platform access</p>;
  }
